...
class SubtitleEngine(BaseModule):
    ...
    def generate_srt_from_word_timestamps(
        ...
        BUGFIX (v3.1.5): word timestamps are narration-relative (start at
        0), but the final video has an intro card playing BEFORE narration
        begins. Without shifting subtitle times to account for that lead-in,
        every caption appears too early — burned over the wrong image,
        consistently "one scene ahead" of what's on screen (confirmed via
        real render screenshots). Pass offset_ms (the intro's actual
        duration in ms) to shift every block's timing to match when
        narration actually starts in the final joined video.
        """
        ...
        offset_ms = int(self._as_float(opts.get("offset_ms"), 0))
        ...
        if offset_ms:
            for block in blocks:
                block["start_ms"] += offset_ms
                block["end_ms"] += offset_ms

        # BUGFIX (v3.2.0): the last block previously ran right up to when
        # narration technically ends — which is also exactly when the
        # crossfade into the (silent, branded) outro card starts blending
        # in. Result: the final caption visibly overlapped the "Thanks for
        # Watching" card for the length of one crossfade. Timing-wise this
        # was correct (confirmed via v3.1.9), but visually undesirable —
        # trim the last block to end when the outro transition BEGINS
        # (crossfade_duration before narration's real end), not when it
        # completes, so the last caption always clears before outro visuals
        # start appearing. Must match export_engine.DEFAULT_TRANSITION
        # duration — same kept-in-sync-deliberately constant as prior fixes.
        if blocks:
            crossfade_duration_ms = 800
            last = blocks[-1]
            trimmed_end = last["end_ms"] - crossfade_duration_ms
            last["end_ms"] = max(trimmed_end, last["start_ms"] + MIN_BLOCK_MS)
        ...
        # PHASE 9: written atomically — the burn-in stage (and any
        # player loading the sidecar) must never see a partially
        # written SRT, and a failed write is reported instead of
        # silently producing an empty/short caption file.
    ...
    def get_style_ffmpeg_params(
        ...
        BUGFIX (v3.1.1): force_style previously never set PlayResX/PlayResY.
        FFmpeg's ``subtitles`` filter converts SRT to ASS internally and, when
        no script resolution is given, can fall back to a small default
        canvas (observed: subtitles filling ~75% of frame height instead of
        a normal ~5%). Explicitly passing the real output resolution here
        makes FontSize scale correctly to the actual video, on every
        machine/ffmpeg build. Defaults to 1920x1080 (the app's standard
        export size) if the caller doesn't know the real resolution yet.
        """
        started = time.perf_counter()
    ...
    def _probe_video_resolution(
        ...
        """Best-effort real width/height of a video file via ffprobe.

        Returns (None, None) on any failure; get_style_ffmpeg_params falls
        back to the 1920x1080 default in that case, which is still correct
        for the app's most common export preset.
        """
        ...
        try:
            ...
            return (width, height)
        except Exception:  # noqa: BLE001 - probe is best-effort, default covers it
            return (None, None)

    def _video_encode_args(self, ffmpeg: str) -> List[str]:
        """Best available video encoder args: hardware first, else software.

        BUGFIX (v3.2.3): was a flat -b:v 8000k for every hardware encoder
        — wasteful for this step specifically, since it re-encodes the
        WHOLE video (including long, low-motion segments like an outro
        card) at one uniform bitrate. Now uses each encoder's own
        quality-based mode (ICQ/CQ/CQP), matching the same fix applied to
        scene rendering — smaller output with no visible quality loss.

        Lightweight, self-contained probe (subtitle_engine doesn't import
        export_engine — Rule A) — checks ffmpeg's advertised encoder list
        for a common hardware candidate. Caches the result on the instance
        so repeated burn_subtitles calls (e.g. multi-part exports) don't
        re-probe every time.
        """
        cached = getattr(self, "_video_encoder_cache", None)
        if cached is None:
            candidates = ["h264_qsv", "h264_nvenc", "h264_amf", "h264_videotoolbox"]
            found = None
            try:
                probe = subprocess.run(
                ...
                found = None
            cached = found
            self._video_encoder_cache = cached
        quality = "21"  # matches this app's default CRF-equivalent target (v3.2.6)
        if cached == "h264_qsv":
            return ["-c:v", cached, "-global_quality", quality, "-look_ahead", "1"]
        if cached == "h264_nvenc":
            return ["-c:v", cached, "-rc", "vbr", "-cq", quality]
        if cached == "h264_amf":
            return ["-c:v", cached, "-rc", "cqp", "-qp_i", quality, "-qp_p", quality]
        if cached == "h264_videotoolbox":
            return ["-c:v", cached, "-q:v", quality]
        return ["-c:v", "libx264", "-crf", quality]

    def _burn_subtitles_two_pass(
        self,
        ffmpeg: str,
        video_path: str | Path,
        vf: str,
        ...
        """Real two-pass libx264 encode: analysis pass, then final encode.

        Returns a response dict on success/failure, or None if the pass-1
        analysis step itself couldn't even start (caller falls back to
        normal single-pass in that case).
        """
        passlog = str(Path(output_path).with_suffix("")) + "_2pass"
        # BUGFIX (found during testing): libx264 rejects combining -crf
        # with -pass — two-pass mode is inherently bitrate-targeted (pass
        # 1 analyzes the content, pass 2 allocates bits to hit that
        # target as efficiently as possible), not quality-targeted like
        # CRF. Use a fixed reasonable target bitrate for a 1080p-class
        # final deliverable instead.
        target_bitrate = "6000k"
        pass1 = [
            str(ffmpeg), "-y", "-i", str(video_path), "-vf", vf,
            "-c:v", "libx264", "-preset", "medium", "-b:v", target_bitrate,
            "-pass", "1", "-passlogfile", passlog,
            "-an", "-f", "null",
            "-",
        ]
        run1 = self._run_ffmpeg(pass1, started)
        if not run1["success"]:
            self.log.warning(
                "Two-pass analysis (pass 1) failed, falling back to "
                "single-pass: %s", run1.get("error"),
            )
            return None
        pass2 = [
            str(ffmpeg), "-y", "-i", str(video_path), "-vf", vf,
        ...
        run2 = self._run_ffmpeg(pass2, started)
        # BUGFIX (found during testing): ffmpeg actually writes
        # "{passlogfile}-0.log" (stream index suffix), not
        # "{passlogfile}.log" — the original cleanup pattern silently
        # missed the real files, leaving stray .log/.log.mbtree files
        # behind after every two-pass render. Glob for the real pattern.
        for stray in Path(passlog).parent.glob(Path(passlog).name + "-*.log*"):
            try:
                stray.unlink()
            except OSError:
                pass
    ...
    def burn_subtitles(
        ...
        FEATURE (v3.2.4): two_pass=True (opt-in, default off) does a real
        two-pass libx264 encode of this final re-encode step — better
        quality-per-MB than single-pass, at the cost of roughly 2x the
        time for this step. Only applies to the software libx264 path
        (hardware encoders here don't support classic two-pass the same
        way); if a hardware encoder is in use, two_pass is silently
        ignored — hardware quality-based encoding is already efficient
        enough that this tradeoff isn't worth forcing software encoding
        just to enable it. Intended for a final deliverable render, not
        quick test renders.
        """
        started = time.perf_counter()
        if not self._enabled:
        ...
        video_width, video_height = self._probe_video_resolution(video_path)
        params = self.get_style_ffmpeg_params(
            style, video_width=video_width, video_height=video_height
        )
        ...
        # PERFORMANCE (v3.2.2): this step re-encodes the ENTIRE final video
        # (not just one scene) to draw subtitles on top — previously always
        # with software libx264, even when scene rendering elsewhere used a
        # hardware encoder. For a long video this is one of the biggest
        # remaining costs, since it scales with total video length. Try the
        # same hardware encoder used for scene rendering first; if it's
        # unavailable or fails at runtime, fall back to software exactly as
        # before (never a correctness risk, only a speed one).
        video_codec_args = self._video_encode_args(str(ffmpeg))
        if two_pass and video_codec_args[1] == "libx264":
            two_pass_result = self._burn_subtitles_two_pass(
                ffmpeg, video_path, vf, output_path, started
            )
            if two_pass_result is not None:
                if not two_pass_result["success"]:
                    return two_pass_result
                return self.make_response(
                    True,
                    {
            ...
            self.log.info(
                "two_pass requested but a hardware encoder (%s) is in use; "
                "hardware quality-based encoding is already efficient, so "
                "two-pass (software-only) is skipped for this render",
                video_codec_args[1],
            )
        command = [
            str(ffmpeg),
        ...
        if not run["success"] and video_codec_args[1] != "libx264":
            self.log.warning(
                "Hardware-encoded subtitle burn failed (%s); retrying with "
                "software libx264",
                run.get("error"),
            )
            command = [
