...
def _drop_enum(name: str) -> None:
    """Drop a native ENUM type, if the backend has one (M6-F3).

    PostgreSQL implements ``sa.Enum`` as a standalone ``TYPE`` that outlives
    ``DROP TABLE``. Because this downgrade did not drop it, the documented
    rollback-then-retry procedure failed on the only supported production
    database:

        alembic downgrade base && alembic upgrade head
        -> DuplicateObject: type "executionstatus" already exists

    SQLite stores enums as VARCHAR and has no type to drop, so this is a no-op
    there. ``checkfirst`` keeps the downgrade idempotent.
    """
    bind = op.get_bind()
...
def downgrade() -> None:
    ...
    # M6-F3: the tables are gone, but on PostgreSQL the enum type they shared
    # is not. Drop it last, once no column can still reference it.
    _drop_enum("executionstatus")
