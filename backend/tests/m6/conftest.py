"""Shared fixtures for the M6 validation suite.

Mirrors ``tests/m5/conftest.py``: pytest resolves ``conftest.py`` per package,
so the M6 package needs its own copy of the auth/client fixtures rather than
importing across sibling test packages. ``auth_settings`` enables
``AUTH_ENABLED`` for the duration of a test and supplies a signing secret, so
the default single-user behaviour asserted by the M0-M4 suites is undisturbed.
"""
